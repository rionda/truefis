...
\emph{approximate} the set of TFIs. This is \fabio{the focus of this work.}
...
false positives while \fabio{\st{achieving a high success rate} retrieving a large fraction of the TFIs.}
...
\begin{definition}\label{def:empvcdim}
  ...
  (VC) dimension of \fabio{$\range$} on $B$}, denoted as $\EVC(\range,B)$ is
...
$1-\delta$\fabio{, according to the following result.}
...
\begin{definition}\label{def:antichain}
	...
	$\mathcal{A}$ of $\mathcal{S}$ such that for any two elements $A' \fabio{\neq} A''$ of
