...
\begin{algorithm}[htbp]
  ...
  \For{$\tau\in\Ds$}{\label{algline:forbegin}
		...
		contain an element with key $I_\tau$} {
			...
			$L[I_\tau]\leftarrow |\mathcal{C}_\tau|$\;\label{algline:assignmentintersection}
			\If{$T$ does not contain any element with key
				$\lfloor\log_2|\mathcal{C}_\tau|\rfloor + 1$}{
					$T[\lfloor\log_2|\mathcal{C}_\tau|\rfloor + 1]\leftarrow \{I_\tau\}$\;
			}\Else{
				$T[\lfloor\log_2|\mathcal{C}_\tau|\rfloor + 1]\leftarrow
				T[\lfloor\log_2|\mathcal{C}_\tau|\rfloor + 1]\cup\{I_\tau\}$\;
			}
		}
  ...
  $d_i=\max\{ d < d_{i-1} ~:~ |\mathsf{T}_d\setminus\mathsf{T}_{d_{i-1}}|\ge d-b_{i-1}\}$\;
	\If{$d_i\le b_{i-1}$}{
		\Return{$d_i$}\;
	}
	$b_i\leftarrow$ size of largest antichain on
	$\mathsf{T}_{d_i}$\;\label{algline:antichain}
	\If{$b_i \ge d_i$}{
		\Return{$d_i$}
	}
