...
def main():
    ...
    # Verify arguments
    if len(sys.argv) != 7: 
        error_exit("Usage: {} DELTA MINFREQ GAP DATASETNAME ORIGRES SAMPLERES\n".format(os.path.basename(sys.argv[0])))
    dataset_name = sys.argv[4]
    orig_res_filename = os.path.expanduser(sys.argv[5])
    if not os.path.isfile(orig_res_filename):
        error_exit("{} does not exist, or is not a file\n".format(orig_res_filename))
    sample_res_filename = os.path.expanduser(sys.argv[6])
    ...
    (eps_vc_dim, eps_emp_vc_dim, returned) = epsilon.epsilon_dataset(2, delta_1, dataset_name) # XXX Temporarily using 2
    ...
    print("large={},sample={},e1={},e2={},e3={},d={},min_freq={},origFIs={}".format(os.path.basename(orig_res_filename),
        os.path.basename(sample_res_filename), epsilon_1, epsilon_2, epsilon_3, delta, min_freq,
        len(orig_res_set)))
    ...
    sys.stderr.write("{}\n".format(",".join((str(i) for i in (os.path.basename(orig_res_filename),
        os.path.basename(sample_res_filename), epsilon_1, epsilon_2, epsilon_3, delta,
